...
public class CircleFindDiameterKnownPerimeter implements IQuestionFactory{
	private int numq = 2;
	Locale loc;
	ResourceBundle bundle;
	
	public CircleFindDiameterKnownPerimeter() {
		this.loc = new Locale("in", "ID");
		initStringFromLocale();
	}
	
	public CircleFindDiameterKnownPerimeter(Locale loc) {
		super();
		this.loc = loc;
		initStringFromLocale();
	}
	...
	public List<Quiz> generateQuizList() {
		...
		for (int i=0;i<numq;i++){
			...
			String question = bundle.getString("geom.shape2d.question.circle.diameter.fromperimeter");
			question = question.replace("#perimeter", ""+Geom.formatMeasurement(shp.getPerimeter()));
