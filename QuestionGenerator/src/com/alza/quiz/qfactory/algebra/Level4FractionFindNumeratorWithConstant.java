...
public class Level4FractionFindNumeratorWithConstant implements IQuestionFactory{

	private int numOfQuestion = 5;
	private Map<Integer, ProblemSkeleton> qMap = new HashMap<Integer, Level4FractionFindNumeratorWithConstant.ProblemSkeleton>();
	private Locale loc;
	private ResourceBundle bundle;
	private ResourceBundle bundleAlgebra,bundleAlgebraSteps;
	
	public Level4FractionFindNumeratorWithConstant(Locale loc){
		this.loc = loc;
		initStringFromLocale();
	}
	public Level4FractionFindNumeratorWithConstant(){
	...
	protected class ProblemSkeleton implements ISingleQuizPrimaryAttributeGenerator{
		...
		ProblemSkeleton(int idx) {
			...
			do {
				...
				mod = a * b % constant;				
			} while (a == b || mod != 0 || a == constant || b == constant);						
		}
		...
		int[] getChoices() {			
			int[] choices = {generateAnswerInt(),a*b - constant,a*b+constant};
			return choices;
		}
		...
		public List<SolutionStep> generateSolutionSteps(){
			...
			exp = left+" \\times v2 = \\frac{VAR}{v2} \\times v2";			
			exp = replaceAllSymbols(exp);
			exp = CommonFunctionAndValues.enclosedWithMathJaxExp(exp);
		...
		public String generateQuestion() {
			String s = "v1 = VAR / v2 + constant";
			if (even) {
				s = "v1 = VAR / v2 - constant";
			}
		...
		public String generateQuestionMathjax() {
			String s = "v1 = \\frac{VAR}{v2} + constant";
			if (even) {
				s = "v1 = \\frac{VAR}{v2} - constant";
			}
			s = replaceAllSymbols(s);
